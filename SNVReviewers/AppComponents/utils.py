...
def plot_fdr_comparison(df, method1, method2, alp):
    ...
    return fig, sig_genes_dict, df_plot
...
def gen_dnds_summary_table(df, dropdown_menu_value):
    ...
    for i, s in enumerate(subsets):
        ...
        if i == 0:
        ...
        elif dropdown_menu_value == "MutSig2CV, dNdScv and DIG":
            ...
            other_methods = dropdown_menu_value_methods.split(" ")

            # keeps only the rows that contain True values for the two methods being analyzed
            is_keep = np.all(df[['SIG_' + m for m in other_methods]] == True, axis=1)
...
def gen_dnds_comparison_plot(
    ...
    comparison_fig, sig_genes_dict, df_significance = plot_fdr_comparison(df, methods_val[0], methods_val[1], alp)
    ...
    return comparison_fig, df_plot_list, df_significance
